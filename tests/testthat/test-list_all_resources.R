...
test_that("filters by package_contains (case-insensitive) and warns when empty", {
  # Pick a common term likely present in package titles; adjust if needed
  data_pkg <- list_all_resources(package_contains = "hospital")
  ...
  # Explicit empty case to assert warning + empty tibble
  expect_warning(
    data_none <- list_all_resources(package_contains = "___no_such_package___"),
    regexp = "No packages found"
  )
  expect_equal(nrow(data_none), 0)
})
...
test_that("combined filtering works", {
  ...
  if (nrow(data_combo) > 0) {
    expect_true(all(grepl("hospital", data_combo$package_name, ignore.case = TRUE)))
    expect_true(all(grepl("admissions", data_combo$resource_name, ignore.case = TRUE)))
  }
})

test_that("input validation: package_contains/resource_contains must be NULL or length-1", {
  # Length > 1 should error
  expect_error(
    list_all_resources(package_contains = c("a", "b")),
    regexp = "length-1"
  )
  expect_error(
  ...
  expect_no_error(list_all_resources(package_contains = NULL))
